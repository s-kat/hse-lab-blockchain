...
describe('ERCToken', function () {
  ...
  beforeEach(async function () {
    this.ercToken = await this.ERCToken.deploy();
  ...
  it('Creates a token with a name', async function () {
    ...
    // expect(await this.ercToken.name()).to.equal('ERCToken');
  });

  it('Creates a token with a symbol', async function () {
    expect(await this.ercToken.symbol()).to.exist;
    // expect(await this.ercToken.symbol()).to.equal('FUN');
  });

  it('Has a valid decimal', async function () {
    expect((await this.ercToken.decimals()).toString()).to.equal('18');
  })
  ...
  it('Transfers the right amount of tokens to/from an account', async function () {
    ...
    await expect(this.ercToken.transfer(this.recipientAddress, transferAmount)).to.changeTokenBalances(
        this.ercToken,
  ...
  it('Emits a transfer event with the right arguments', async function () {
    ...
    await expect(this.ercToken.transfer(this.recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals)))
        .to.emit(this.ercToken, "Transfer")
  ...
  it('Allows for allowance approvals and queries', async function () {
    ...
    expect((await this.ercToken.allowance(this.recipientAddress, this.ownerAddress))).to.equal(ethers.utils.parseUnits(approveAmount.toString(), this.decimals));
  });
  ...
  it('Emits an approval event with the right arguments', async function () {
    ...
    await expect(this.signerContract.approve(this.ownerAddress, ethers.utils.parseUnits(approveAmount.toString(), this.decimals)))
        .to.emit(this.ercToken, "Approval")
  ...
  it('Allows an approved spender to transfer from owner', async function () {
    ...
    await this.ercToken.transfer(this.recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await this.signerContract.approve(this.ownerAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await expect(this.ercToken.transferFrom(this.recipientAddress, this.ownerAddress, transferAmount)).to.changeTokenBalances(
        this.ercToken,
        [this.ownerAddress, this.recipientAddress],
        [transferAmount, -transferAmount]
      );
  ...
  it('Emits a transfer event with the right arguments when conducting an approved transfer', async function () {
    ...
    await this.ercToken.transfer(this.recipientAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await this.signerContract.approve(this.ownerAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals))
    await expect(this.ercToken.transferFrom(this.recipientAddress, this.ownerAddress, ethers.utils.parseUnits(transferAmount.toString(), this.decimals)))
        .to.emit(this.ercToken, "Transfer")
  ...
  it('Allows allowance to be increased and queried', async function () {
    ...
    const previousAllowance = await this.ercToken.allowance(this.recipientAddress, this.ownerAddress);
    ...
    expect((await this.ercToken.allowance(this.recipientAddress, this.ownerAddress))).to.equal(expectedAllowance);
  });
